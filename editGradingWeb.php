	...
	<section class="scroll-container" role="main">
		...
		<div class="row">
			...
			<div class="large-9 medium-8 columns">
				...
				<h4 class="subheader"> Editar ponderacion a cada respuesta </h4>
						...
						foreach($questions as $question) {
							if ($level == $questionsByLevel[$question->id]){
								...
								foreach($answersForQuestion as $a){
									...
									$selectedGrade = $c->getGrade($answerId, $webInCompetence->id);
									$options = "";
									for ($i=1-$level; $i <= 0; $i++) {
										$strSelected = "";
										if ($i == $selectedGrade)
											$strSelected = " selected ";
										$options .= "<option value='$i' $strSelected>$i</option>";
									}
									echo "<tr>";
									if ($a[0]->correct == 1){
...
<script>
	...
	function gradeWeb() {
		...
		for(var i=0; i<len; i++) {
			...
			$.post( "controls/doAction.php", { action:"editGradeWeb", cId: cId, webId: webId, data: data})
			.done(function( data ) {
				console.log(data);
